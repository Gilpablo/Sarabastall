...
<nav aria-label="breadcrumb">
  ...
  <div class="row d-flex justify-content-center text-center mx-0 mt-3 mb-2">
      <div class="col-12">
  ...
  <thead>
    ...
    <tbody id="tbody">
      <?php foreach ($datos['beca']as $beca): ?>
        ...
        <button type="button" <?php echo $estadoBoton1?> onclick="modalPagar1(<?php echo $beca->idBeca ?>,<?php echo $datos['tipoBeca']->idTipoBeca?>)" 
        ...
        <button type="button" <?php echo $estadoBoton2?> onclick="modalPagar2(<?php echo $beca->idBeca ?>,<?php echo $datos['tipoBeca']->idTipoBeca?>,<?php echo $beca->primerPago?>,<?php echo $beca->Importe?>)" 
        data-bs-toggle="modal" data-bs-target="#modal2" class="w-sm-100  btn btn-outline-warning btn-lg"><img id="icono" src="<?php echo RUTA_URL?>/img/prestamo.png">
        </button>
...
<div class="modal fade" id="modal">
    ...
    <div class="modal-dialog modal-dialog-centered modal-xl">
      ...
      <div class="modal-content">
        ...
        <!-- Modal Header -->
        <div class="modal-header">
          <h2>PRIMER PAGO BECA <img id="icono" src="<?php echo RUTA_URL?>/img/prestamo.png"></h2>
        </div>
        ...
        <div class="mb-3 col-12 mt-2">
          ...
          <div class="mb-3 col-12 mt-2">
            ...
            <div class="modal-footer">
              <input type="hidden" id="idBeca2" name="idBeca">
...
<div class="modal-dialog modal-dialog-centered modal-xl">
  ...
  <div class="modal-content">
    ...
    <!-- Modal Header -->
    <div class="modal-header">
      <h2>SEGUNDO PAGO BECA <img id="icono" src="<?php echo RUTA_URL?>/img/prestamo.png"></h2>
    </div>
                        
    <div class="container col-6">
    <form method="post" action="<?php echo RUTA_URL ?>/beca/add_pago2/<?php echo $beca->idBeca?>">
    <div class="mb-3 col-12 mt-2">
                          
      <h4 id="titulopago">Segundo Pago</h4>
                          
      <div class="mb-3 col-12 mt-2">
        <label for="nombre" class="form-label">Importe</label>
        <input type="number" class="form-control" name="importePago1" id="importepago2"required disabled>
        </div>
        <input type="hidden" name="importePago" id="importepago3" required>
        <input type="hidden" id="valuepago" value="1" name="pago">
                       
              
        </div>
        <div class="modal-footer">
          <input type="hidden" id="idBeca3" name="idBeca">
          <input type="hidden" id="tipoBeca3" name="tipoBeca" value="<?php echo $beca->idTipoBeca?>">
          <button type="submit" class=" btn btn-success btn-lg" >Realizar pago</button>
...
<script>
  ...
  function modalPagar1(idBeca,idTipoBeca){
...
function modalPagar2(idBeca,idTipoBeca,primerPago,importe){
    
    document.getElementById("idBeca3").value= idBeca;
    document.getElementById("tipoBeca3").value= idTipoBeca;
    let segundoPago=importe-primerPago;
    console.log(segundoPago);
    document.getElementById("importepago2").value=segundoPago;
    document.getElementById("importepago3").value=segundoPago;
}
